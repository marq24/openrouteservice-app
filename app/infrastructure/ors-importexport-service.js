  ...
  /**
    ...
    (
      ...
      let toTcx = (name, speedInKmPerH) => {
        ...
        let turnInstructions = orsRouteService.data.features[orsRouteService.getCurrentRouteIdx()].properties.segments;
        let [majorV, minorV, patchV] = version.split(".");
        let garminPartNumber = `EMA-CBERR-Y0`;
        let tcx = {
        ...
        /*for (const segment of turnInstructions) {
          ...
          for (const step of segment.steps) {
            ...
            step.type;
            if (turnType == 7) {
                ttString = ttString + ":" + aStep.getInt("exit_number");
            }

            step.instruction; // Turn left onto <b>Goethestraße</b> || Enter the roundabout and take the 2nd exit onto <b>Platzstraße, K 9</b>
            step.name;        // Target Street Name... (or empty)
